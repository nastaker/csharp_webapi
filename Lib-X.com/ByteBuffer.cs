...
namespace ilabHelper
{
    ...
    public class ByteBuffer : Stream
    {
        ...
        public Int64 GetLong()
        {
            ...
            long tmp = (long)((bytes[0] << 56) | (bytes[1] << 48) | (bytes[2] << 40) | bytes[3] << 32 | (bytes[4] << 24) | (bytes[5] << 16) | (bytes[6] << 8) | (bytes[7]));
            //long tmp2 = ((bytes[7] << 56) | (bytes[6] << 48) | (bytes[5] << 40) | bytes[4] << 32 | (bytes[3] << 24) | (bytes[2] << 16) | (bytes[1] << 8) | (bytes[0]));
            return tmp;
        }
